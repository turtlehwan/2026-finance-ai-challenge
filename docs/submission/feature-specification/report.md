...
하는지 모르겠다`는 비식별 합성 상황이다. 사용자는 보험가입증서와 진료자료를
불러오고, Agent는 가입 당시 보험약관을 선택한다. 수술 여부가 빠져 있으면 그
자리에서 질문하고, 답변 뒤에는 근거 조항·쪽수·준비 서류·보험회사에 물어볼 질문을
한 흐름으로 보여 준다. 이 흐름은 지급 가능성을 확정하지 않는다.
...
| 필수 근거 동반 조회 | 선택 버전에 저장된 정의·지급·제한·면책·서류 근거 반환 | 근거 약관 | 근거 묶음 도구 (`graph_retriever`) | 필수 유형 누락 |
