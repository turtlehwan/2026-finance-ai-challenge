...
```mermaid
flowchart TB
    O["공식 약관 O\n2개 상품·3개 버전"] --> D["결정론적 버전·근거 그래프"]
    S["합성 또는 사용자 문서"] --> M["추출·마스킹"]
...
그림 2. 실제 데이터와 선택형 AI의 입력·출력·권한 경계.
...
그림 3. 심사자가 90초 안에 성공 경로와 실패 경로를 함께 확인하는 검증 순서.
...
| --- | --- |
| 실제 LangGraph 실행과 질문 후 재개 | 보험금 지급 정확도·지급 가능성 |
| 2개 상품·3개 버전의 계약일 기반 선택 | 타 보험사·전체 상품 일반화 |
| 공식 출처·버전·쪽수·필수 근거 유형의 존재 | 법률·의학적 의미의 최종 해석 정확성 |
| 근거 부족·버전 불일치 시 안전 중단 | 모든 OCR 문서의 변환 정확성 |
| 합성 50건·15건의 실행 계약 점검 | 실제 고객 분포·성과·보험금 수령 증가 |
