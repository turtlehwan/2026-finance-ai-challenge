...
- 보험증권·진료서류·사고 설명을 연결해 사용자가 **확인해 볼 보장 항목**을
  선별한다.
...
- 정보가 부족하면 추정하지 않고 Agent가 필요한 사실을 질문한 뒤 기존
  사건 상태에서 분석을 재개한다.
- 결과를 단순 답변이 아닌 `사건 → 진단 → 보장 항목 → 정의 → 지급 → 면책 →
...
4. **대화가 화면을 바꾸는 적응형 비서 UI**
   채팅 답변을 길게 쌓지 않고, 요청에 따라 Claim Evidence Map·공식 원문·
   Action Pack·Agent trace가 주 화면으로 이동한다. 사건부터 조항과 필요
   서류까지의 도달 경로와 공식 실행 경로를 필요한 순간에만 크게 보여준다.
...
적용 보험약관을 사용한다. 합성 보험증권에는 `무배당 생활재해보장특약Ⅱ 2504` 가입
...
지원 차이와 외부 인식 서비스 전송 가능성을 입력창에 고지한다. 음성 원본을
서비스 서버에 저장하지 않고, 음성이 없어도 텍스트와 빠른 요청으로 같은 흐름을
완료할 수 있다.
...
- `확인 권장`: 보험증권·사건·보험약관 근거가 연결됐으나 보험사 최종 확인이 필요
