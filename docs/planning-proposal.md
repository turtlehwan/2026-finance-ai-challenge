...
- 금융감독원·국가법령정보센터의 현행 질병·상해보험 표준약관을 공통 기준으로
  함께 회수하고, 출처 URL·시행일·페이지·해시를 결과에 공개한다.
...
- **Case Analyst Agent**: 사용자 목표와 사고·진단·기존 청구 여부 구조화,
  다음 도구 선택, 누락 정보 질문
- **Policy Curator Agent**: 공식 원문·적용일·해시를 확인하고 조항 구조화 및
  지식 그래프 갱신 후보를 생성한다. 예선 MVP의 승인 버튼은 운영 인덱스를
  자동 변경하지 않는다.
...
- KB손해보험 공식 상품목록(약관) 공시 페이지(민간 보험사 공시 구조 확인용)
...
계약·안전성 통과율이며, 독립 holdout과 보험사 지급 이력은 없다.
