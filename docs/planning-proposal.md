...
그림 1. 공식 사실·제품 가설·구현 증거의 연결. 공식 통계를 곧바로 서비스의
시장 규모나 성과로 전환하지 않는다.
...
그림 2. 실제 LangGraph 실행 순서. 화면은 실행된 노드와 대기·중단 이유를
React Flow로 표시한다.
...
그림 3. 공식 실제 데이터, 합성·사용자 문서, 선택형 AI의 처리 경계.
...
그림 4. 성공 결과뿐 아니라 질문·재개와 실패 시 안전 중단까지 확인하는 검증 계약.
