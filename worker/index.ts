/** Cloudflare Worker entry point and request-scoped Workers AI tools. */
import {
 ...
 */
async function handleMaskedFactInterpretation(request: Request, env: Env) {
  ...
  try {
    const raw = (await env.AI.run(AI_FACT_MODEL, {
      messages: [
        {
          role: "system",
          content:
 ...
 */
async function handleAiDocumentConversion(request: Request, env: Env) {
  if (request.method !== "POST") {
    return json({ error: "POST 요청만 허용합니다." }, 405)
  }
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/api/ai/interpret") {
      return handleMaskedFactInterpretation(request, env)
    }
    if (url.pathname === "/api/documents/ai-convert") {
      return handleAiDocumentConversion(request, env)
    }
