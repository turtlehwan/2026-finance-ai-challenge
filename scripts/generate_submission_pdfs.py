...
def title_page(title, subtitle, s):
    return [
        ...
        Spacer(1, 10 * mm),
        Paragraph("문서 상태: 예선 제출용 검토 초안 · 기준일: 2026-08-06", s["small"]),
        Spacer(1, 2 * mm),
...
def build_proposal(path):
    ...
    story += [table([
        ...
        ["실제성·안전", "우체국보험 2개 상품·3개 버전의 공식 근거 사용 · 지급 판단·청구 대행은 하지 않음"],
    ], [44 * mm, 130 * mm], s)]
    story += section("2. 아이디어 기획 핵심내용", s)
    story += [callout("<b>핵심 결론</b> · 보험금 확인의 어려움은 조회 버튼이 아니라, 내 문서·치료 사실·가입 당시 보험약관을 대조해 무엇을 확인할지 정리하는 데서 발생합니다.", s)]
    ...
    story += [paragraph("보험금 미청구는 조회 채널의 부재만으로 설명되지 않습니다. 금융위원회는 보험금 발생 사실을 알지 못하거나 이자 조건을 오해하는 경우를 원인으로 제시했습니다. 2026년 7월 발표 기준 찾아가야 할 숨은보험금은 10.3조 원이며, 2025년에만 약 80만 건·3조 2,470억 원이 환급됐습니다.", s)]
    story += bullet([
        "보험금 청구 절차가 불편하다는 응답: 단일응답 39.7%, 복수응답 52.5%",
        "보험금 지급 과정 안내가 부족하다는 응답: 54.7%",
        "보험금 지급 내역 안내가 부족하다는 응답: 51.4%",
    ], s)
    story += [Paragraph("출처: 금융위원회 ‘숨은보험금 10.3조 원’ 보도자료 · 보험연구원 ‘보험소비자 행태조사’ 본문. 수치는 발표·조사 문항의 기준을 그대로 유지했습니다.", s["small"])]
    story += [table([
        ["관찰된 불편", "확인 근거", "서비스가 줄이는 공백"],
        ["지급 사유·보장 항목의 존재를 알기 어려움", "금융위원회가 숨은보험금 원인으로 보험금 발생 사실을 알지 못하는 경우를 제시", "사건·진단·보장 항목을 연결해 확인할 항목부터 좁힘"],
        ["절차·지급 안내를 따라가기 어려움", "보험연구원 조사: 청구 절차 불편·지급 과정 안내 부족", "조항 근거, 미확인 조건, 서류, 공식 경로를 한 결과에 배치"],
