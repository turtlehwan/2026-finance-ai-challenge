...
Node.js `>=24.0.0`이 필요하다. 브라우저에서 `http://localhost:3000`을 열고
`골절 · 2504` → `이 사례 분석하기` → `아니오` → `답변 반영하고 결과 보기`
순서로 실행한다.
...
테스트는 LangGraph 분석 API, 50건 결정론적 회귀 fixture, 15건 규칙 재확인
경계 사례, 공식 근거 미연결 안전 중단 2건, 표준약관 provenance API,
PolicyOps 승인 payload 검증까지 실행한다. 이 지표는 실제 보험금 지급 정확도가
아니다.
...
- 비용이 발생하는 AI API 2개는 Cloudflare Rate Limiting 바인딩으로 경로별 분당
  30회로 제한한다. 이는 사용자 인증이나 정확한 과금 계측을 대신하지 않는다.
